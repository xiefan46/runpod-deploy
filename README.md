...
| 想做 | 脚本 | 说明 |
|------|------|------|
| 基础环境 | `setup_env.sh` | 系统工具 + huggingface_hub/wandb 等 + 验证 GPU |
| profiling 工具(nsys/ncu) | `install_nsight.sh` | 装 Nsight Systems/Compute(系统级 + kernel 级) |
| 推理框架 FastVideo | `setup_fastvideo.sh` | clone + 安装(STA/DMD/self-forcing,M2/M4) |
| 微调框架 DiffSynth-Studio | `setup_diffsynth.sh` | clone + 安装(Wan LoRA/全量微调,M1) |
| Matrix-Game / Wan | `setup_mg.sh` | clone(推理对照,M2) |
| 下载模型权重 | `download_models.sh [repo]` | 默认 Wan2.1-T2V-1.3B |
...
bash setup_env.sh
# M0 profiling:
cd profiling && python run_basic.py && python run_torchprof.py && python run_mfu.py
# M1 微调:
...
- **profiling/ 目录**:Module 0 的可跑脚本(mini 模型 + 各工具),换靶子(真实模型)时复用同样的工具。
